...
WmService.prototype.handleWord = function(doc, word){
	var that = this;
	var w = WordBuilder.gen(word);
	col.findAndModify(doc, {$push, {words : w}} function(err, doc){
		if(err)
			that.emit('dberr', 'Database error occured when handling new word!');
		else {
